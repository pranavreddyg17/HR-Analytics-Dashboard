...
import { planAiCalendarWorkflow } from "@/lib/server/ai-workflows"
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await planAiCalendarWorkflow(await request.json(), actor))
  } catch (error) {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "The scheduling plan could not be created." }, { status: 500 })
  }
}
