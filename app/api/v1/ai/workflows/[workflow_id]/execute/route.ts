...
import { executeAiCalendarWorkflow } from "@/lib/server/ai-workflows"
...
export async function POST(request: Request, context: { params: Promise<{ workflow_id: string }> }) {
  try {
    ...
    return NextResponse.json(await executeAiCalendarWorkflow(workflowId, actor, request))
  } catch (error) {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "The calendar event could not be created." }, { status: 500 })
  }
}
