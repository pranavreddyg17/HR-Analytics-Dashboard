...
export async function GET(request: Request) {
  try {
    ...
    const status = error instanceof PeopleError ? error.status : error instanceof Error && error.message === "AUTH_REQUIRED" ? 401 : error instanceof OnboardingUnavailableError ? 503 : 500
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to load onboarding operations." },
