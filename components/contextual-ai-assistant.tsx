...
export function ContextualAiAssistant({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  if (!open) return <button type="button" className="assistant-launcher" onClick={() => onOpenChange(true)} aria-label={`Ask AI about ${context.label}`}><span>Ask AI</span></button>
  return <div className="assistant-drawer-layer" role="dialog" aria-modal="true" aria-label="Contextual AI assistant">
