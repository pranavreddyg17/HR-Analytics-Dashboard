    ...
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ left: -18, right: 8, top: 8, bottom: 4 }}>
          ...
          <YAxis tick={{ fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} />
    ...
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={rows} layout="vertical" margin={{ left: 28, right: 12, top: 6, bottom: 4 }}>
          ...
          <XAxis type="number" tick={{ fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} />
          <YAxis type="category" dataKey="label" width={104} tick={{ fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} />
          <Tooltip content={<ChartTooltip />} />
...
function DataTable({ rows, columns }: { rows: Array<Record<string, unknown>>; columns: Array<{ key: string; label: string }> }) {
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      ...
      <div className="border-t border-border bg-muted/30 px-3 py-2 text-meta text-muted-foreground">Showing {Math.min(rows.length, 60)} of {rows.length} filtered rows</div>
    </div>
  )
}
...
export function WorkforceDashboard({ initialView = "executive" }: { initialView?: View }) {
  ...
  return <div className="flex flex-col gap-5">
    {demoDomains.length > 0 && <div className="flex flex-col gap-3 rounded-md border border-warning/25 bg-warning/5 p-4 sm:flex-row sm:items-center"><AlertTriangle className="size-5 shrink-0 text-warning"/><div className="flex-1"><p className="text-sm font-medium">Demo operational data is active</p><p className="text-xs text-muted-foreground">{demoDomains.map((item)=>item.domain).join(", ")} use correlated synthetic records linked by employee ID to the 1,470-row IBM dataset. Imported records remain separate.</p></div><Button nativeButton={false} variant="outline" size="sm" render={<Link href="/data"/>}>Import HR data <ArrowRight className="size-3.5"/></Button></div>}

    <Card className="gap-4 p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3 border-b border-border/70 pb-3"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-lg bg-primary/10 text-primary"><SlidersHorizontal className="size-4" /></span><div><p className="text-sm font-semibold">Dashboard filters</p><p className="text-meta text-muted-foreground">Applied to charts, records, and exports</p></div></div><Button variant="ghost" size="sm" onClick={()=>setFilters(initialFilters)}><FilterX className="size-4"/>Reset</Button></div>
