...
import { AlertTriangle, ArrowRight, Bot, Building2, CalendarDays, Download, FilterX, GraduationCap, Sparkles, TrendingDown, UserPlus, Users } from "lucide-react"
...
type View = "executive" | "hiring" | "attrition" | "leave" | "training" | "promotions"
...
function DomainView({ view, data, setDepartment }: { view: Exclude<View, "executive">; data: WorkforceAnalytics; setDepartment: (value: string) => void }) {
  ...
  return <>
    <div className="grid gap-4 md:grid-cols-4"><Metric label="Promotions" value={number(data.promotions.total)} hint="Selected period" icon={ArrowRight}/><Metric label="Promotion rate" value={number(data.promotions.rate, "%")} hint="Of active employees" icon={Sparkles} tone="success"/><Metric label="Time to promotion" value={number(data.promotions.averageMonthsToPromotion, " mo")} hint="Average since prior move" icon={CalendarDays}/><Metric label="No promotion 3+ years" value={number(data.promotions.withoutPromotionOver36Months)} hint="Review career paths and data" icon={AlertTriangle} tone="warning"/></div>
...
export function WorkforceDashboard() {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      <div className="flex flex-1 gap-1 overflow-x-auto rounded-xl border border-border bg-card p-1">{views.map((item)=><button key={item.id} onClick={()=>setView(item.id)} className={cn("whitespace-nowrap rounded-lg px-3 py-2 text-xs font-medium transition-colors",view===item.id?"bg-primary text-primary-foreground":"text-muted-foreground hover:bg-muted hover:text-foreground")}>{item.label}</button>)}</div>
    ...
    {view === "executive" ? <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Active employees" value={number(data.kpis.activeEmployees)} hint={`${data.kpis.totalEmployees} workforce records`} icon={Users}/><Metric label="Attrition rate" value={number(data.kpis.attritionRate,"%")} hint={`${data.attrition.totalExits} recorded exits`} icon={TrendingDown} tone="warning"/><Metric label="Training completion" value={number(data.kpis.trainingCompletionRate,"%")} hint={`${data.training.requiringMandatoryTraining} mandatory gaps`} icon={GraduationCap} tone="success"/><Metric label="Promotion rate" value={number(data.promotions.rate,"%")} hint={`${data.promotions.total} recorded promotions`} icon={Sparkles}/></div>
      <div className="grid gap-4 xl:grid-cols-[1.35fr_1fr]"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="size-4 text-primary"/>Decision brief</CardTitle><CardDescription>Rule-grounded insights from the same filtered records used by every chart</CardDescription></CardHeader><CardContent className="space-y-3">{data.executiveInsights.map((insight,index)=><div key={index} className="flex gap-3 rounded-lg bg-muted/40 p-3"><span className="font-mono text-xs text-primary">0{index+1}</span><p className="text-sm leading-relaxed">{insight}</p></div>)}<Button nativeButton={false} variant="outline" render={<Link href="/ai-agents"/>}><Bot className="size-4"/>Ask the MCP analytics agent</Button></CardContent></Card><Card><CardHeader><CardTitle>Data readiness</CardTitle><CardDescription>Imported records replace demo rows domain by domain</CardDescription></CardHeader><CardContent className="space-y-2">{data.status.map((item)=><div key={item.domain} className="flex items-center gap-3 rounded-lg border border-border/70 p-2.5"><span className={cn("size-2 rounded-full",item.mode==="imported"?"bg-success":item.mode==="demo"?"bg-warning":"bg-muted-foreground")}/><span className="flex-1 text-sm capitalize">{item.domain}</span><span className="font-mono text-xs tabular-nums">{item.count}</span><span className="w-16 text-right text-[10px] uppercase text-muted-foreground">{item.mode}</span></div>)}</CardContent></Card></div>
