...
export function TimeOffWorkspace({ canRequestLeave }: { canRequestLeave: boolean }) {
  ...
  return <WorkspacePage>
    ...
    </div></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[960px] text-left text-body"><thead className="bg-muted/40 text-label font-semibold text-muted-foreground"><tr>{["Employee", "Leave", "Dates", "Department", "Status", "Coverage", ""].map((heading) => <th key={heading} className="px-4 py-2.5">{heading}</th>)}</tr></thead><tbody>{visible.map((row) => <tr key={row.id} className="border-t border-border/70 hover:bg-muted/20"><td className="px-4 py-3"><Link href={`/leaves?request=${encodeURIComponent(row.id)}`} className="font-semibold hover:text-primary">{row.employeeName}</Link><p className="text-meta text-muted-foreground">{row.employeeId}</p></td><td className="px-4 py-3">{row.leaveType} · {row.leaveDays}d</td><td className="px-4 py-3 whitespace-nowrap">{dateLabel(row.startDate)} — {dateLabel(row.endDate)}</td><td className="px-4 py-3">{row.department}</td><td className={cn("px-4 py-3 font-semibold", statusTone(row.status))}>{row.status}</td><td className="px-4 py-3 text-muted-foreground">{row.coverage.approvedAway} approved · {row.coverage.pendingRequests} pending</td><td className="px-4 py-3 text-right">{row.canDecide ? <Button size="xs" variant="outline" onClick={() => openDecision(row, "Approved")}>Review</Button> : <span className="text-muted-foreground">—</span>}</td></tr>)}</tbody></table></div>{!visible.length && <p className="p-10 text-center text-body text-muted-foreground">No leave requests match this view.</p>}<div className="border-t border-border bg-muted/20 px-4 py-2.5 text-meta text-muted-foreground">Showing {visible.length} of {data.requests.length} requests</div></CardContent></Card>
