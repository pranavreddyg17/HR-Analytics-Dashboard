...
function LeaveTable({ rows, people, reviewer, employees, busyId, selectedId, onDecision }: { rows: LeaveRecord[]; people: Map<string, string>; reviewer: Reviewer; employees: Map<string, EmployeeRecord>; busyId: string | null; selectedId: string | null; onDecision: (row: LeaveRecord, decision: "Approved" | "Rejected") => void }) {
  ...
  useEffect(() => {
    if (!selectedId || !paged.some((row) => row.id === selectedId)) return
    ...
    <Card id="leave-register" className="gap-0 scroll-mt-24 overflow-hidden py-0 shadow-none">
      <CardHeader className="gap-4 border-b border-border px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground"/><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(0) }} placeholder="Search employee or leave type" className="h-9 w-full rounded-md border border-border bg-background pl-9 pr-3 text-xs outline-none focus:ring-2 focus:ring-ring/30 sm:w-64"/></label>
          <select value={status} onChange={(event) => { setStatus(event.target.value); setPage(0) }} className="h-9 rounded-md border border-border bg-background px-3 text-xs outline-none"><option value="">All statuses</option>{statuses.map((item) => <option key={item}>{item}</option>)}</select>
        </div>
      </CardHeader>
      <CardContent className="px-0 pb-0">
...
export function TimeOffWorkspace({ canRequestLeave, reviewer }: { canRequestLeave: boolean; reviewer: Reviewer }) {
  ...
  return <WorkspacePage>
    ...
    <MetricStrip metrics={[{ label: "Away today", value: new Set(data.leave.currentlyAway.map((row) => row.employee_id)).size.toLocaleString(), detail: "Approved leave" }, { label: "Pending", value: data.leave.pending.toLocaleString(), detail: `${pendingForReview.length} available to review` }, { label: "Approved days", value: data.leave.totalDays.toLocaleString(), detail: `${data.leave.approved} requests` }]}/>
