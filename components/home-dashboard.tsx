...
export function HomeDashboard({ analytics, inbox, people }: HomeDashboardProps) {
  ...
  const upcomingEvents = [
    ...people.filter((person) => person.hire_date >= today && person.hire_date <= thirtyDays).map((person) => ({ id: `start-${person.employee_id}`, date: person.hire_date, label: `${person.display_name} starts`, detail: `${person.job_title} · ${person.department}`, action: "View employee", href: `/people/${encodeURIComponent(person.employee_id)}` })),
    ...analytics.leave.upcoming.filter((leave) => leave.start_date >= today && leave.start_date <= thirtyDays && leave.approval_status.toLowerCase() === "approved").map((leave) => ({ id: `leave-${leave.id}`, date: leave.start_date, label: `${leave.leave_type} leave begins`, detail: `${people.find((person) => person.employee_id === leave.employee_id)?.display_name ?? leave.employee_id} · ${leave.department}`, action: "Review coverage", href: `/time-off?request=${encodeURIComponent(leave.id)}` })),
    ...openItems.filter((item) => item.type === "training" && item.dueDate && item.dueDate >= today && item.dueDate <= thirtyDays).map((item) => ({ id: `due-${item.id}`, date: item.dueDate as string, label: `${item.title} due`, detail: item.person || item.owner, action: "Open assignment", href: item.reviewHref })),
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 pb-10">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="gap-0 py-0 shadow-none"><CardHeader className="border-b border-border px-5 py-4"><CardTitle>Deadlines and exceptions</CardTitle><CardDescription>Open work that is overdue or approaching its due date.</CardDescription></CardHeader><CardContent className="divide-y divide-border p-0">{alerts.map((alert) => <Link key={alert.label} href={alert.href} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/20"><span className="min-w-0 flex-1 text-sm font-medium">{alert.label}</span><span className={cn("text-sm font-semibold tabular-nums", alert.count > 0 ? "text-foreground" : "text-muted-foreground")}>{alert.count}</span></Link>)}</CardContent></Card>

        <Card className="gap-0 py-0 shadow-none"><CardHeader className="border-b border-border px-5 py-4"><CardTitle>Changes in the last seven days</CardTitle><CardDescription>Recorded operational changes, not all-time totals.</CardDescription></CardHeader><CardContent className="divide-y divide-border p-0">{changes.map((change) => <div key={change.label} className="flex items-center gap-4 px-5 py-3.5"><span className="min-w-0 flex-1 text-sm font-medium">{change.label}</span><span className="text-sm font-semibold tabular-nums">{change.value}</span></div>)}</CardContent></Card>
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="gap-0 py-0 shadow-none"><CardHeader className="flex-row items-start justify-between gap-3 border-b border-border px-5 py-4"><div><CardTitle>Manager actions awaiting completion</CardTitle><CardDescription>Open the assigned request to record the required decision or update.</CardDescription></div><Link href="/inbox?view=managers" className="text-xs font-semibold hover:underline">View all</Link></CardHeader><CardContent className="divide-y divide-border p-0">{managerActions.length ? managerActions.map((item) => <Link key={`${item.type}-${item.id}`} href={itemHref(item)} className="flex gap-4 px-5 py-3.5 hover:bg-muted/20"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.title}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{item.person ? `${item.person} · ` : ""}{item.nextAction}</span></span><span className="shrink-0 text-right"><span className="block text-xs font-semibold">Review request</span><span className={cn("mt-0.5 block text-meta", item.slaStatus === "overdue" ? "font-semibold text-destructive" : "text-muted-foreground")}>{dueLabel(item)}</span></span></Link>) : <p className="px-5 py-8 text-center text-sm text-muted-foreground">No manager-owned actions are open.</p>}</CardContent></Card>

        <Card className="gap-0 py-0 shadow-none"><CardHeader className="border-b border-border px-5 py-4"><CardTitle>Upcoming workforce events</CardTitle><CardDescription>Open the related record to prepare onboarding, leave coverage, or training follow-up.</CardDescription></CardHeader><CardContent className="divide-y divide-border p-0">{upcomingEvents.length ? upcomingEvents.map((event) => <Link key={event.id} href={event.href} className="flex gap-4 px-5 py-3.5 hover:bg-muted/20"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{event.label}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{event.detail}</span></span><span className="shrink-0 text-right"><span className="block text-xs font-semibold">{event.action}</span><span className="mt-0.5 block text-meta text-muted-foreground">{readableDate(event.date)}</span></span></Link>) : <p className="px-5 py-8 text-center text-sm text-muted-foreground">No recorded workforce events are scheduled in the next 30 days.</p>}</CardContent></Card>
      </div>
