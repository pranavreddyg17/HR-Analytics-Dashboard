...
export function InsightsWorkspace() {
  ...
  return <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-5 pb-10">
    ...
    <section aria-labelledby="program-title"><div className="mb-3"><h2 id="program-title" className="text-base font-semibold">Workforce programs</h2><p className="mt-0.5 text-xs text-muted-foreground">Time off, learning, and internal mobility measures.</p></div><div className="grid gap-3 md:grid-cols-3"><Programme href="/time-off" label="Time off" metric={`${new Set(data.leave.currentlyAway.map((row) => row.employee_id)).size} away today`} detail={`${data.leave.pending} pending requests · ${data.leave.totalDays} approved days`}/><Programme href="/learning" label="Learning" metric={`${data.training.completionRate}% complete`} detail={`${data.training.requiringMandatoryTraining} mandatory gaps · ${data.training.totalHours} assigned hours`}/><Programme href="/people?tenure=mobility" label="Internal mobility" metric={`${data.promotions.rate}% promotion rate`} detail={`${data.promotions.total} promotions · ${data.promotions.withoutPromotionOver36Months} employees due for review`}/></div></section>
