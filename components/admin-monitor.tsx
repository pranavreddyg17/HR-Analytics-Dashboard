...
export function AdminMonitor({ initialData }: { initialData: AdminMonitorData }) {
  ...
  return <div className="workspace-stack">
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      ...
      <section className="surface-card overflow-hidden"><div className="border-b border-border px-5 py-4"><h2 className="text-section-title">Azure cost</h2></div>{cost ? <div><div className="p-5"><p className="text-label text-muted-foreground">Month to date</p><p className="mt-1 text-kpi tabular-nums">{money(cost.monthToDate, cost.currency)}</p></div><div className="divide-y divide-border border-t border-border">{cost.byService.slice(0, 8).map((row) => <div key={row.service} className="flex items-center justify-between gap-3 px-5 py-3"><span className="text-body">{row.service}</span><span className="text-body tabular-nums">{money(row.cost, cost.currency)}</span></div>)}</div></div> : <ProviderUnavailable reason={data.cost.status === "unavailable" ? data.cost.reason : "Cost data is unavailable."} />}</section>
    </div>
  </div>
}
