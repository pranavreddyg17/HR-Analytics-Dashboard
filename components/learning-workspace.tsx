...
export function LearningWorkspace({ actor, people }: { actor: WorkflowActorContext; people: ManagedEmployee[] }) {
  ...
  return <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-5 pb-10">
    <section className="relative overflow-hidden rounded-[1.75rem] border border-slate-800 bg-[#0d1424] px-5 py-6 text-white shadow-[0_18px_60px_rgba(15,23,42,0.14)] sm:px-7"><div className="pointer-events-none absolute -right-20 -top-28 size-72 rounded-full bg-violet-400/10 blur-3xl"/><div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-violet-300"><GraduationCap className="size-3.5"/>Learning workspace</p><h1 className="mt-2 text-3xl font-bold tracking-[-0.05em] sm:text-4xl">Build skills. Close compliance gaps.</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-400">Assign programmes, follow up on overdue work, record completions, and understand participation from one learning register.</p></div>{actor.canAssignTraining && assignablePeople.length > 0 && <Button onClick={() => { setError(""); setAssignOpen(true) }} className="bg-violet-300 text-slate-950 hover:bg-violet-200"><Plus className="size-4"/>Assign training</Button>}</div><div className="relative mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-slate-800 pt-4 text-[10px] text-slate-400"><span className="inline-flex items-center gap-1.5"><Database className="size-3.5 text-violet-300"/>Live records only</span><span>{data.training.rows.length} assignments in this view</span><span>Updated {new Date(data.generatedAt).toLocaleString()}</span></div></section>
