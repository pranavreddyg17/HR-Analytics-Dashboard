...
export function LearningWorkspace({ actor, initialData }: { actor: WorkflowActorContext; initialData: LearningOperations }) {
  ...
  const [status, setStatus] = useState(selectedAssignmentId ? "all" : ["attention", "overdue", "incomplete", "completed", "all"].includes(requestedStatus ?? "") ? requestedStatus as string : "attention")
