...
export function LearningWorkspace({ actor }: { actor: WorkflowActorContext }) {
  ...
  const [assignment, setAssignment] = useState({ employeeId: "", courseId: "", dueDate: dateAfterToday(14), hours: "", note: "" })
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/hr/learning${requestQuery ? `?${requestQuery}` : ""}`, { cache: "no-store", signal: controller.signal })
      ...
      .then((result) => {
        setData(result)
        setAssignment((current) => ({ ...current, employeeId: result.people.some((person) => person.employeeId === current.employeeId) ? current.employeeId : result.people[0]?.employeeId ?? "", courseId: result.courses.some((item) => item.id === current.courseId) ? current.courseId : result.courses[0]?.id ?? "" }))
        setError("")
      })
  ...
  function openAssign() {
    setError("")
    setAssignment((current) => ({ ...current, employeeId: current.employeeId || data?.people[0]?.employeeId || "", courseId: current.courseId || data?.courses[0]?.id || "" }))
    setDialog("assign")
  }
  ...
  async function submitAssignment(event: React.FormEvent) {
    ...
    setError("")
    try {
      const response = await fetch("/api/v1/hr/learning", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ employeeId: assignment.employeeId, courseId: assignment.courseId, dueDate: assignment.dueDate, hours: assignment.hours ? Number(assignment.hours) : undefined, note: assignment.note }) })
